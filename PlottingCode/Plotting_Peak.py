...
"""
Change lines 107 and 108 to save file as a WIMP event or ER event
"""

#Plotting largest peaks 
import random
import os
import datetime
import pytz
...
class PlotBase(plugin.OutputPlugin):
    ...
    def finalize_plot(self, event_number=0):
        ...
        if self.output_dir:
            ...
            filename = 'C:/WIMP%d' % event_number             #Change file location    
       	    #filename = 'C:/ER%d' % event_number          #Change file location  
    ...
    def plot_waveform(self, event,left=0, right=None, pad=0,show_peaks=False, show_legend=True, log_y_axis=False,scale=1,ax=None):
...
# TODO: change from lines to squares
        ...
        for hit in hits:
            ...
            hit['channel'],                                               # Y
                ...
                (1 if hit['is_rejected'] else 0),       
                # Is rejected? If so, will make green
            ])
        if len(result) != 0:
